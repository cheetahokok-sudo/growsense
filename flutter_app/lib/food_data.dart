...
class FoodItem {
  ...
  final String category; // chicken | beef | pork | fish | seafood | egg | dairy | plant | composite
  ...
  FoodItem.fromJson(Map<String, dynamic> j)
      : id = j['id'] as String,
        ...
        proteinPer100g =
            ((j['per100g']?['protein_g']) as num?)?.toDouble() ?? 0,
...
/// Same category order as the PWA's browse-modal tabs.
...
Future<List<FoodItem>> loadFoodReference() async {
  ...
  _cache = [
    for (final e in list) FoodItem.fromJson(e as Map<String, dynamic>),
  ];
  return _cache!;
}
